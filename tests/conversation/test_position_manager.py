...
def test_open_position(pm):
    pos = pm.open_position("123", "BTCUSDT", Side.LONG, 67500.0, 10.0, stop_loss=65000.0)
...
def test_get_active_positions(pm):
    pm.open_position("123", "BTCUSDT", Side.LONG, 67500.0, 10.0, stop_loss=65000.0)
    pm.open_position("123", "ETHUSDT", Side.SHORT, 3800.0, 5.0, stop_loss=4000.0)
...
def test_get_all_active_positions(pm):
    pm.open_position("123", "BTCUSDT", Side.LONG, 67500.0, 10.0, stop_loss=65000.0)
    pm.open_position("456", "ETHUSDT", Side.SHORT, 3800.0, 5.0, stop_loss=4000.0)
...
def test_open_position_rejects_zero_price(pm):
    with pytest.raises(ValueError, match="진입가는 0보다 커야 합니다"):
        pm.open_position("123", "BTCUSDT", Side.LONG, 0, 10.0, stop_loss=65000.0)
    with pytest.raises(ValueError, match="진입가는 0보다 커야 합니다"):
        pm.open_position("123", "BTCUSDT", Side.LONG, -100, 10.0, stop_loss=65000.0)


def test_open_position_rejects_zero_leverage(pm):
    with pytest.raises(ValueError, match="레버리지는 1~125 사이여야 합니다"):
        pm.open_position("123", "BTCUSDT", Side.LONG, 67500.0, 0, stop_loss=65000.0)
    with pytest.raises(ValueError, match="레버리지는 1~125 사이여야 합니다"):
        pm.open_position("123", "BTCUSDT", Side.LONG, 67500.0, -5, stop_loss=65000.0)
    with pytest.raises(ValueError, match="레버리지는 1~125 사이여야 합니다"):
        pm.open_position("123", "BTCUSDT", Side.LONG, 67500.0, 126, stop_loss=65000.0)


def test_close_by_symbol_closes_only_first(pm):
    pm.open_position("123", "BTCUSDT", Side.LONG, 67500.0, 10.0, stop_loss=65000.0)
    pm.open_position("123", "BTCUSDT", Side.SHORT, 68000.0, 5.0, stop_loss=70000.0)
...
def test_open_position_rejects_no_stop_loss(pm):
    with pytest.raises(ValueError, match="손절가"):
