...
class TestPortfolioGuard:
    def test_allows_when_under_limit(self):
        guard = PortfolioGuard(max_positions=3)
    ...
    def test_blocks_daily_loss_limit(self):
        guard = PortfolioGuard(daily_loss_limit_pct=5.0)
        # Record loss: 60 USDT loss on 1000 USDT margin = 6%
        guard.record_realized_loss(60.0, 1000.0)
        result = guard.check_can_open([], "BTCUSDT")
        assert result.allowed is False
        assert "일일 손실 한도" in result.reason

    def test_blocks_monthly_drawdown(self):
        guard = PortfolioGuard(daily_loss_limit_pct=99.0, monthly_drawdown_limit_pct=20.0)
        # Record loss: 250 USDT loss on 1000 USDT margin = 25%
        guard.record_realized_loss(250.0, 1000.0)
        result = guard.check_can_open([], "BTCUSDT")
        assert result.allowed is False
    ...
    def test_record_loss_accumulates(self):
        ...
        guard.record_realized_loss(10.0, 500.0)
        guard.record_realized_loss(20.0, 500.0)
        assert guard._daily_realized_loss == 30.0
        assert guard._daily_total_margin == 1000.0
        assert guard._monthly_realized_loss == 30.0
        assert guard._monthly_total_margin == 1000.0

    def test_reset_daily(self):
        guard = PortfolioGuard()
        guard.record_realized_loss(50.0, 1000.0)
        guard.reset_daily()
        assert guard._daily_realized_loss == 0.0
        assert guard._daily_total_margin == 0.0
    ...
    def test_get_daily_status(self):
        guard = PortfolioGuard(daily_loss_limit_pct=5.0)
        guard.record_realized_loss(30.0, 1000.0)
        status = guard.get_daily_status()
        assert status["realized_loss_usdt"] == 30.0
        assert status["total_margin_usdt"] == 1000.0
